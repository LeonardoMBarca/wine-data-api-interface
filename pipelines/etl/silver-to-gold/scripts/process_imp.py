...
def main():
    ...
    csv_path = [fr"{os.path.join("data", "silver-layer", f"Imp{i}.csv")}" for i in ["Frescas", "Passas", "Suco", "Vinhos", "Espumantes"]]
    gold_path = "data/gold-layer"
    output_file = [fr"{os.path.join(gold_path, f"Imp{i}.csv")}" for i in ["Frescas", "Passas", "Suco", "Vinhos", "Espumantes"]]

    try:        
        for i, e in enumerate(csv_path):
            if output_file[i] == fr"{os.path.join(gold_path, f"ImpSuco.csv")}":
